...
from airtable_to_figma.template import (
    PdfPage,
    PdfReportConfig,
    TemplateConfig,
    TemplateOutput,
...
def run_pdf_report_pipeline(
    ...
    raw_label = ""
    if report.filename_label_field:
        raw_label = str(fields.get(report.filename_label_field) or "").strip()
    safe_label = re.sub(r"[^\w\s-]", "", raw_label).strip()
    safe_label = re.sub(r"[\s_]+", "-", safe_label) or "report"
    filename = f"{safe_label}-{report.filename_suffix}.pdf" if report.filename_suffix else f"{safe_label}.pdf"
    airtable.upload_attachment(
        record_id=record_id,
